...
const Favorites = () => {
  ...
  const fetchFavorites = async () => {
    try {
      ...
      const [postsData, musicData] = await Promise.all([getMyFavoritePosts(1), getMyFavoriteMusic(1)]);
              ...
              {filteredFavorites.map((item) => (
                <div key={`${item.type}-${item.id}`} className={styles.favoriteCard}>
                  <div className={styles.favoriteInfo}>
                    <div className={styles.typeLabel}>{item.type === 'post' ? '📝 게시물' : '🎵 음악'}</div>
                    <h3 className={styles.favoriteTitle}>{item.title}</h3>
                    {item.author && <p className={styles.favoriteAuthor}>작성자: {item.author}</p>}
                    <p className={styles.favoriteDate}>{new Date(item.createdAt).toLocaleDateString('ko-KR')}</p>
                  </div>
                  <div className={styles.favoriteActions}>
                    <button onClick={() => handleItemClick(item)} className={styles.viewButton}>
                      보기
                    </button>
                    <button onClick={() => handleRemove(item.id, item.type)} className={styles.removeButton}>
                      제거
                    </button>
                  </div>
                </div>
